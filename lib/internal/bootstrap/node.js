...
// one quantum.
{
  ...
  const preset = (resolutionNs, nestingClamp, timerGridMs) => ObjectFreeze({
    __proto__: null, resolutionNs, nestingClamp, timerGridMs,
  });
  const presets = ObjectFreeze({
    __proto__: null,
    // Stock Node: raw nanosecond floats, 1ms minimum timers.
    stock: preset(0, false, 0),
    // What a stock Node on Windows looks like: QPC-shaped floats on the
    // 64Hz scheduler tick.
    winemu: preset(100, false, 15.625),
    // Blink: 100us TimeClamper + DOMTimer nesting.
    chrome: preset(100000, true, 0),
    // Blink shape riding the Windows tick.
    winchrome: preset(100000, true, 15.625),
  });
