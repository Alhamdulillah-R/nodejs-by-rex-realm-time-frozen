...
// A timer created while a timer callback runs inherits that callback's level
// plus one; from kMaxTimerNestingLevel on, any delay below kMinimumIntervalMs
// is raised to it, and a top-level zero delay stays zero instead of being
// rounded up to 1ms.  The flag is queried through the binding on every use:
// this module is part of the startup snapshot, so a module-level constant
// would freeze whatever the snapshot builder's environment said.
const kNestingLevel = Symbol('nestingLevel');
const kMaxTimerNestingLevel = 5;
