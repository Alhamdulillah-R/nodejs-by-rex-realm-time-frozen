...
// Blink DOMTimer nesting semantics, enabled with REX_TIMER_NESTING_CLAMP=1
// (dom_timer.cc, kSpecCompliantMaxTimerNestingLevel).  A timer created while
// a timer callback runs inherits that callback's level plus one; once the
// level exceeds kMaxTimerNestingLevel any delay below kMinimumIntervalMs is
// raised to it (so the 7th timer of a setTimeout(0) chain is the first one
// delayed), and a top-level zero delay stays zero instead of being rounded up
// to 1ms.  The flag is queried through the binding on every use: this module
// is part of the startup snapshot, so a module-level constant would freeze
// whatever the snapshot builder's environment said.
const kNestingLevel = Symbol('nestingLevel');
const kMaxTimerNestingLevel = 6;
const kMinimumIntervalMs = 4;
let currentTimerNestingLevel = 0;

// Run `fn` as if it were a fresh browser task at the given DOMTimer nesting
// level.  A framework replaying recorded input events from inside its own
// timer chain would otherwise hand the target an ever-growing nesting level
// (every replayed event is one level deeper), which a real page never sees:
// input dispatch always starts a new task at level 0.
function runWithTimerNestingLevel(level, fn) {
  const prior = currentTimerNestingLevel;
  currentTimerNestingLevel = level;
  try {
    return fn();
  } finally {
    currentTimerNestingLevel = prior;
  }
}

function getTimerNestingLevel() {
  return currentTimerNestingLevel;
}
...
class Timeout {
  ...
  constructor(callback, after, args, isRepeat, isRefed) {
    // Browsers treat a missing or invalid delay as 0; stock Node uses 1.
    const timerNestingClamp = binding.nestingClampEnabled();
    const minimumAfter = timerNestingClamp ? 0 : 1;
    if (after === undefined) {
      after = minimumAfter;
    } else {
    ...
    if (!(after >= minimumAfter && after <= TIMEOUT_MAX)) {
      if (after > TIMEOUT_MAX) {
        process.emitWarning(`${after} does not fit into` +
                            ' a 32-bit signed integer.' +
                            `\nTimeout duration was set to ${minimumAfter}.`,
      ...
      after = minimumAfter; // Schedule on next tick, follows browser behavior
    }

    const requestedAfter = after;
    if (timerNestingClamp) {
      this[kNestingLevel] = currentTimerNestingLevel + 1;
      if (this[kNestingLevel] > kMaxTimerNestingLevel &&
          after < kMinimumIntervalMs) {
        after = kMinimumIntervalMs;
      }
    }

    this._idleTimeout = after;
    binding.traceTimer(0, requestedAfter, after,
                       this[kNestingLevel] === undefined ? 0 : this[kNestingLevel]);
    this._idlePrev = this;
    this._idleNext = this;
...
function getTimerCallbacks(runNextTicks) {
  ...
  function listOnTimeout(list, now) {
    ...
    while ((timer = L.peek(list)) != null) {
      ...
      const timerNestingClamp = binding.nestingClampEnabled();
      const priorNestingLevel = currentTimerNestingLevel;
      if (timerNestingClamp)
        currentTimerNestingLevel = timer[kNestingLevel];
      binding.traceTimer(1, timer._idleTimeout, now - timer._idleStart,
                         timer[kNestingLevel] === undefined ? 0 : timer[kNestingLevel]);
      ...
      try {
        ...
        currentTimerNestingLevel = priorNestingLevel;
        if (timer._repeat && timer._idleTimeout !== -1) {
          if (timerNestingClamp) {
            // Blink raises the level on every repeat and clamps the interval
            // once the level passes the maximum.
            if (timer[kNestingLevel] <= kMaxTimerNestingLevel)
              timer[kNestingLevel]++;
            if (timer[kNestingLevel] > kMaxTimerNestingLevel &&
                timer._repeat < kMinimumIntervalMs) {
              timer._repeat = kMinimumIntervalMs;
            }
          }
...
module.exports = {
  ...
  kHasPrimitive,
  kNestingLevel,
  runWithTimerNestingLevel,
  getTimerNestingLevel,
  initAsyncResource,
  setUnrefTimeout,
